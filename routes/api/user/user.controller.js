...
exports.resetpassword = (req, res) => {
  ...
  User.findOne({ emailAddress: req.body.emailAddress }, function (err, user) {
    ...
    if (!user) {
      ...
      var url = process.env.clientUrl + '/setpassword/?token=' + user._id;
      ...
      var emailText = `<p>Hi ${user.firstName}</p><p>Please <a href="${url}">click here</a> to reset your password and continue using our portal</p><p>Regards</p>` + process.env.projectName

      var mailOptions = {
        from: 'Tax Management <admin@precedentonline.com>',
        to: userEmail,
...
exports.emailLogin = (req, res) => {
  User.findOne({ emailAddress: req.body.emailAddress }).then(user => {
    ...
    if (user) {
      var url = process.env.clientUrl + '/login/email?token=' + user._id;

      var userEmail = user.emailAddress;
      // var emailText = 'please click on the below link for the forget password link';
      emailText += '<p><a href="' + url + '">click here</a>';
      var emailText = `<p>Hi ${user.firstName}</p><p>Please <a href="${url}">click here</a> to login to your portal</p><p>Regards</p>` + process.env.projectName
